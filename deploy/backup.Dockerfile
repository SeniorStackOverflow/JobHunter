...
COPY --chmod=0555 deploy/backup_entrypoint.py /usr/local/bin/backup_entrypoint.py
COPY --chmod=0555 deploy/backup_entrypoint.py /usr/local/bin/job-agent-backup-entrypoint
COPY --chmod=0555 deploy/restore.sh /usr/local/bin/job-agent-restore
COPY --chmod=0555 deploy/restore_entrypoint.py /usr/local/bin/job-agent-restore-entrypoint
