...
После обработки входящего звонка от работодателя, лучше-усилие-отправка отправляет
summary в Telegram. Соединение исходящее HTTPS только к зафиксированному хосту
`https://api.telegram.org`; `TELEGRAM_BOT_TOKEN` хранится как `SecretStr` и никогда
не логируется. Сообщение содержит компанию, вакансию, текст summary из LLM и ссылку
на панель просмотра — не содержит сырой номер звонящего и не экспортирует
`confirmed_facts`. Доставка best-effort, отключена по умолчанию, и сбой никогда не
влияет на обработку звонка.
