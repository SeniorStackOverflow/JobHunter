...
## Telegram post-call notification (Phase 2b)

После обработки входящего звонка от работодателя отдельная задача очереди `phone`
отправляет status-aware карточку в Telegram. Соединение исходящее HTTPS только к
зафиксированному хосту `https://api.telegram.org`; редиректы запрещены,
`TELEGRAM_BOT_TOKEN` хранится как `SecretStr` и никогда не логируется. Карточка
содержит только компанию, вакансию, безопасные значения фактов, короткие цитаты и
ссылку на панель просмотра — не содержит сырой номер звонящего, сырой SMS-текст,
provider errors или disputed values как подтверждённые.

Состояния доставки `pending`, `retrying`, `sent`, `failed` и `disabled` хранятся
вместе с revision, числом попыток, временем следующей попытки и Telegram
`message_id`. Ответы 429 и определённые 5xx используют ограниченный backoff;
ошибка транспорта после отправки запроса становится terminal
`ambiguous_delivery=true`, потому что сообщение могло дойти. Telegram не имеет
idempotency key: если процесс завершится после передачи запроса (в том числе
между ответом Telegram и фиксацией результата) до записи ambiguous/sent state,
возможна повторная отправка после истечения lease. Сбой доставки не
изменяет состояние звонка, summary или фактов. При отключённом Telegram сеть не
вызывается, а pending-запись получает состояние `disabled`.

## Проверка телефонных звонков (Phase 2b production readiness)

После автоотвеченного звонка Celery выполняет три независимых строгих прохода
llmRouter: extractor, verifier и arbiter. Дата, время, часовой пояс, формат и
адрес остаются `candidate` до подтверждения входящим SMS или оператором. Состояние
звонка `high_confidence` означает согласованные модели и доказательство в
транскрипте/аудиоклипе; `confirmed` требует подтверждения источником; `conflict`,
`unknown` и сбой обработки дают `needs_review`. `summary_state=done` не означает,
что факты подтверждены.

Входящие SMS читаются из PhoneGate и импортируются идемпотентно по внешнему ID.
Синхронизация и повторная обработка используют bounded lease и revision CAS: сбой
PhoneGate оставляет предыдущие факты, а следующая успешная история безопасно
продолжает импорт. Повтор одного и того же SMS не создаёт новую ревизию.

Короткие WAV-доказательства хранятся под `PHONE_EVIDENCE_DIR`, ограничены
`PHONE_EVIDENCE_MAX_CLIPS_PER_CALL`, `PHONE_EVIDENCE_MAX_TOTAL_MB` и
`PHONE_EVIDENCE_RETENTION_DAYS`; ошибка получения клипа не прерывает звонок.
В логах не появляются сырые транскрипты, SMS и номера. Оператор проверяет очередь
`needs_review` в `Звонки`, сверяет исходную фразу и доказательство, затем использует
аудируемое исправление факта или связывание SMS. Ручное подтверждение хранит
оператора, старое и новое значение.

DEV запускает этот контур только с явными флагами `PHONE_SUMMARY_LLM_ENABLED` и
`TELEGRAM_ENABLED`; реальный GSM canary и реальные внешние сервисы включаются
отдельно через `ENABLE_REALCALL_TESTS=true`. Phase 2b не делает realtime LLM
диалог, календарную запись, исходящие звонки или исходящие SMS.
