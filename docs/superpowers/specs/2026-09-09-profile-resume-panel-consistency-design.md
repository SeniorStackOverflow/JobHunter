...
- Не форсируем перегенерацию писем заявок, уже стоящих в очереди. Новые заявки
  получают подпись сразу; заявка в `PENDING_REVIEW` с актуальной оценкой получит
  новую подпись только при следующей полной переподготовке (новая оценка,
  деактивация/активация резюме, изменившая привязку, и т. п.). Правку `save_profile`,
  которая бы принудительно прогоняла `prepare()` по всем не-терминальным заявкам
  профиля, сознательно не добавляем — автоотправка выключена по умолчанию, оператор
  видит каждое письмо перед отправкой, а синхронный fan-out в POST-форме с побочным
  пересчётом policy/резюме/контакта имеет крайние случаи. Ограничение фиксируется в
  `README.md`.
...
- `prepare_pending_applications` (`app/applications/service.py:392`) делает полную
  переподготовку (с пересборкой письма через `generate_letter`) только когда заявка
  отсутствует, в статусе `PREPARED`, либо её `match_evaluation_id` устарел. Заявка в
  `PENDING_REVIEW` с актуальной оценкой получает максимум policy-only refresh — тело
  письма не перестраивается, на смену `profile_fingerprint` этот таск не реагирует.
  Форсированной миграции старых писем не добавляем (см. §2.2).
