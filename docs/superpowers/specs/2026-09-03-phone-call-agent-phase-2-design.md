...
2. **`phone_listen_silence_timeout_seconds`** default (20 s) — long enough that a
   thoughtful caller is not cut off, short enough that the call does not drag. A live
   acceptance call (2026-09-05) tried lowering it to 4 s and found that too aggressive
   in the other direction; Phase 2b revisits end-of-turn detection as part of its
   turn-taking design rather than tuning this one constant in isolation.
